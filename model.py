...
#         print(f"Shape of x: {x.shape}")  # e.g., [batch, channels_x, height, width]
#         print(f"Shape of c: {c.shape}")  # e.g., [batch, channels_c]
...
#         return g * f + (1 - g) * x
    ...
    def forward(self, x, c):
        ...
        # Check and adjust if batch sizes mismatch
        if x.size(0) != c.size(0):
            # Assuming 'c' is the tensor that might have incorrect batch size
            # Resize 'c' to match 'x's batch size by selecting the first 'x.shape[0]' entries
            print(f"Mismatched batch sizes found. Adjusting 'c' from {c.shape} to match 'x'.")
            c = c[:x.shape[0], ...]

        # Print shapes for debugging
        # print(f"Shape of x: {x.shape}")  # e.g., [batch, channels_x, height, width]
        # print(f"Shape of c: {c.shape}")  # e.g., [batch, channels_c, height, width]

        # Concatenate transformed feature map
        f = self.main(torch.cat((x, c), dim=1))

        # Compute alpha mask and the final output
        g = (1 + self.attention(f)) / 2
        return g * f + (1 - g) * x
